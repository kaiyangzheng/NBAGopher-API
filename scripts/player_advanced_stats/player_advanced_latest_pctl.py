    ...
    for player in player_data:
        if (player['isActive']):
            ...
            response = requests.request(
                'DELETE', BASE + f"player/advanced_latest_pctls/{player_id}")
            headers = {'Content-Type': 'application/json'}
            response = requests.request(
                'POST', BASE + f"player/advanced_latest_pctls/{player_id}", data=player_advanced_latest, headers=headers)
            print(response.text)
