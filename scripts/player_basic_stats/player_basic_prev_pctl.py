...
def get_team_gp(team_id):
    ...
    while i < len(team_list):
        new_team_list.append(team_list[i].replace('*', ''))
        i += 1
    df['Team'] = new_team_list
    ...
    for player in player_data:
        if player['isActive']:
            ...
            response = requests.request(
                'DELETE', BASE + f"player/basic_prev_pctls/{id}")
            headers = {'Content-Type': 'application/json'}
            response = requests.request(
                'POST', BASE + f"player/basic_prev_pctls/{id}", data=data, headers=headers)
            print(response.text)
