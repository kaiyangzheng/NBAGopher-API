"""
Puts and updates data into player basic career stats database
"""
import requests
import json
...
def main():
    for player in player_data:
        if (player['isActive']):
            ...
            try:
                topg = str(
                    round(float(player_stats['turnovers'])/float(player_stats['gamesPlayed']), 2))
            except:
                topg = '0'
            data = json.dumps({
            ...
            response = requests.request(
                'DELETE', BASE + f"player/basic_career/{player_id}")
            headers = {'Content-type': 'application/json'}
            response = requests.request(
                'POST', BASE + f"player/basic_career/{player_id}", data=data, headers=headers)
            print(response.text)
